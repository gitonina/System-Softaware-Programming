...
.while_begin:           #     // del ciclo para mayor eficiencia
    ...
    # El valor de p está temporalmente en el registro t0
    # No puede hacer más trabajo que la comparación (no puede usar ret)

    lw      a0, 0(t0)    # Carga la dirección de la primera cadena en a0
    lw      a1, 4(t0)    # Carga la dirección de la segunda cadena en a1

    mv	a5,a0
	lbu	a4,0(a0)   #cargo el valor del primer caracter de la cadena en a4
	beq	a4,zero,.L8
	li	a0,0   #contador de palabras para p[0]
	li	a6,32 #espacio vacio
	j	.L7
.L12:
	addi	a0,a0,1
	lbu	a4,0(a5)
...
.P1: 
	...
	lbu	a4,0(a1)
	beq	a4,zero,.R7
	li	a1,0  #contador de palabras para p[1]
	j	.R5
.R1:
	addi	a1,a1,1
	lbu	a4,0(a5)
...
.end:
    ...
    # Calcula y almacena la diferencia en t1
    sub     t1, a0, a1   # Calcula la diferencia entre los contadores de palabras de las dos cadenas
